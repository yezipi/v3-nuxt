...
<template>
  <div class="yzp-article-list">
    ...
    <ul v-if="articles && articles.rows" class="yzp-article-ul">
      <li v-for="(item) in articles.rows" :key="item.id" class="yzp-article-item">
        <nuxt-link :to="setAticleLink(item.id, item.type)" class="yzp-article-item-block">
          <div class="yzp-article-item-cover">
            <base-yzp-image class="yzp-article-item-img" :src="item.cover"></base-yzp-image>
          </div>
          <div class="yzp-article-item-info yzp-box">
            <span v-if="!keywords" class="yzp-article-item-title color-primary">{{ item.title }}</span>
